...
flowchart TD
    ...
    style P4 fill:#93C5FD,color:#1E3A8A
    style P5 fill:#BFDBFE,color:#1E3A8A
    style P6 fill:#DBEAFE,color:#1E3A8A
    style P7 fill:#EFF6FF,color:#1E3A8A
```
...
quadrantChart
    ...
    x-axis Low Domain Knowledge --> High Domain Knowledge
    y-axis Low Network Access --> High Network Access
    quadrant-1 Strong Fit
    quadrant-2 Learn Fast
    quadrant-3 Pivot
    quadrant-4 Partner Up
    You: [0.85, 0.70]
```
...
```mermaid
flowchart TD
    A["Open with context\n'I'm researching a problem, not selling anything'"] --> B["Past behavior questions\n'Walk me through the last time you experienced X'"]
    B --> C["Pain depth questions\n'How often does this happen? What do you do about it?'"]
    C --> D["Current workaround audit\n'What tools/people do you currently use?'"]
    D --> E["Cost of problem\n'What does this cost you in time/money/stress?'"]
    E --> F["Ideal world question\n'If this was magically solved, what would that look like?'"]
```
...
flowchart TD
    ...
    C2["Cause 2"] --> ROOT
    C3["Cause 3"] --> ROOT
    E1["Effect 1"] --> |"leads to"| C1
    E2["Effect 2"] --> |"leads to"| C2
    E3["Effect 3"] --> |"leads to"| C3
```
...
```mermaid
quadrantChart
    title Idea Evaluation: Impact vs Simplicity
    x-axis Low Simplicity --> High Simplicity
    y-axis Low Impact --> High Impact
    quadrant-1 Develop Here ✅
...
```mermaid
flowchart LR
    MH["Must Have\n(MVP core)"] --> SH["Should Have\n(v1.1)"]
    SH --> NH["Nice to Have\n(v2.0)"]
    NH --> OPT["Optional\n(Later)"]

    style MH fill:#1E40AF,color:#fff
    style SH fill:#2563EB,color:#fff
    style NH fill:#93C5FD,color:#1E3A8A
    style OPT fill:#DBEAFE,color:#1E3A8A
```
...
```mermaid
flowchart TD
    A["Core Product/Service\n(solves main pain)"]
    B["Bonus 1\n(removes secondary friction)"]
    C["Bonus 2\n(accelerates time to value)"]
    D["Risk Reversal\n(guarantee, free trial, pilot)"]
    E["Urgency / Scarcity\n(founding price, limited spots)"]
    F["THE OFFER"]
    A --> F
    B --> F
    C --> F
...
```mermaid
flowchart TD
    A["Where does your ICP already spend time?"]
    A --> B["Organic Channels\n(free, relationship-based)"]
    A --> C["Paid Channels\n(scalable, measurable)"]
    B --> D["Personal network\nCommunity groups\nLinkedIn / Social\nReferrals\nPartnerships"]
...
```mermaid
flowchart LR
    A["1 Pilot Customer\n(your network)"] --> B["3–5 Beta Customers\n(white-glove service)"]
    B --> C["10 Paying Customers\n(PMF signal)"]
    C --> D["100 Customers\n(repeatable motion)"]
```
...
```mermaid
flowchart LR
    A["Deploy to customer"] --> B["Collect feedback\n(structured + unstructured)"]
    B --> C["Prioritize fixes\n(pain vs. nice-to-have)"]
    C --> D["Build / Fix"] --> A
```
...
```mermaid
flowchart LR
    A["Lead Generation"] --> B["Qualification"] --> C["Demo / Discovery"]
    C --> D["Proposal / Offer"] --> E["Onboarding"] --> F["Delivery"]
    F --> G["Retention & Upsell"] --> H["Referral"]
```
...
```mermaid
flowchart LR
    M1["Awareness\n→ Reach / Impressions"] --> M2["Acquisition\n→ Leads / Signups"]
    M2 --> M3["Activation\n→ First Value Moment"] --> M4["Retention\n→ Churn / NPS"]
    M4 --> M5["Revenue\n→ MRR / ARR"] --> M6["Referral\n→ NPS Promoters"]
```
...
flowchart TD
    S1["$0–$1M ARR\nFounder-led Sales\nManual Everything\nProve the Model"]
    S2["$1M–$5M ARR\nHire Sales + CS\nContent + SEO Kicks In\nProductize Delivery"]
    S3["$5M–$20M ARR\nBuild Sales Team\nChannel Partnerships\nExpand ICP Slightly"]
    S4["$20M–$100M ARR\nNew Markets / Verticals\nProduct-Led Growth\nM&A or International"]

    S1 --> S2 --> S3 --> S4

    style S1 fill:#1E40AF,color:#fff
    style S2 fill:#2563EB,color:#fff
    style S3 fill:#60A5FA,color:#1E3A8A
    style S4 fill:#DBEAFE,color:#1E3A8A
```
...
```mermaid
flowchart LR
    A["Beachhead\n(Calgary small condos)"] --> B["Adjacent Segment\n(medium condos, HOAs)"]
    B --> C["Geographic Expansion\n(Western Canada)"]
    C --> D["New Verticals\n(strata, co-ops, small landlords)"]
...
flowchart TD
    ...
    G0 -->|Yes| G1 -->|Yes| G2 -->|Yes| G3 -->|Yes| G4 -->|Yes| G5 -->|Yes| G6 -->|Yes| G7
    G0 -->|No| R0["Revisit Idea / Pivot"]
    G1 -->|No| R1["More Interviews"]
    G2 -->|No| R2["Redesign Solution"]
    G3 -->|No| R3["Rework Offer or Exit"]
    G4 -->|No| R4["Test New Channels/Messages"]
    G5 -->|No| R5["Refine Onboarding + Product"]
    G6 -->|No| R6["Fix Processes First"]
    G7 -->|No| R7["Optimize Unit Economics"]
```
